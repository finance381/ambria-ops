...
// money paid out (vendor payments/deductions, salary payments/adjustments) and money
// collected in (event collections, extra-plate collections). Each source tags its rows
// with a payment mode (cash/bank) — entries with no mode aren't real money movement
// (e.g. plain point issuances) and are excluded here.
var TYPE_META = {
...
function PaymentsLedger({ profile }) {
  ...
  async function load() {
    ...
    var [ledgerRes, walletRes] = await Promise.all([
      supabase
        .from('ledger_entries')
    ...
    var walletRows = walletRes.data || []

    // Resolve vendor / employee party names
    var vendorIds = []
    var employeeIds = []
    ledgerRows.forEach(function (r) {
      var list = r.ledger_type === 'vendor' ? vendorIds : employeeIds
    ...
    var [vRes, pRes] = await Promise.all([
      ...
      employeeIds.length > 0 ? supabase.from('profiles').select('id, name').in('id', employeeIds) : Promise.resolve({ data: [] }),
    ])
    var vendorNames = {}; (vRes.data || []).forEach(function (v) { vendorNames[v.id] = v.name })
    var employeeNames = {}; (pRes.data || []).forEach(function (p) { employeeNames[p.id] = p.name })

    // Resolve EPC back-links so collections can be split from plain event collections
    var walletIds = walletRows.map(function (w) { return w.id })
    var epcByWalletTx = {}
    if (walletIds.length > 0) {
      var { data: epcRows } = await supabase.from('extra_plate_collections')
        .select('id, event_id, wallet_tx_id')
        .in('wallet_tx_id', walletIds)
      ;(epcRows || []).forEach(function (e) { if (e.wallet_tx_id) epcByWalletTx[e.wallet_tx_id] = e })
    }

    // Resolve event names for collections (plain: reference_id is the event id; EPC: via extra_plate_collections.event_id)
    var eventIds = []
    walletRows.forEach(function (w) {
    ...
    if (eventIds.length > 0) {
      var { data: evRows } = await supabase.from('events').select('id, event_name').in('id', eventIds)
      ;(evRows || []).forEach(function (e) { eventNames[e.id] = e.event_name })
    }

    var combined = []
    ledgerRows.forEach(function (r) {
      var meta = TYPE_META[r.ref_type] || { label: r.ref_type, direction: 'out', cls: 'bg-gray-50 text-gray-700 border-gray-200' }
      var partyName = r.ledger_type === 'vendor' ? (vendorNames[r.party_id] || '—') : (employeeNames[r.party_id] || '—')
      combined.push({
        key: 'le:' + r.id,
        date: r.entry_date,
  ...
  useRealtime(['ledger_entries', 'wallet_transactions', 'extra_plate_collections'], function () { load() })
