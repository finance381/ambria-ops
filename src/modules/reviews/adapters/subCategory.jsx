...
async function fetchDetail(sourceId) {
  var res = await supabase.from('sub_categories')
    .select('*, categories(name), profiles:added_by(name, email)')
...
function renderDetailBody(row) {
  ...
  return (
    <div className="space-y-3">
      <div>
        <p className="text-[10px] font-bold text-gray-400 uppercase">Proposed sub-category name</p>
        <p className="text-base font-bold text-gray-900">{titleCase(row.name)}</p>
      </div>
      <div>
        <p className="text-[10px] font-bold text-gray-400 uppercase">Parent category</p>
        <p className="text-sm text-gray-900">{row.categories ? row.categories.name : '—'}</p>
      </div>
      <div>
      ...
      <p className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
        Rejecting this permanently deletes the pending entry — there's no persistent "rejected" state for sub-category names.
      </p>
    </div>
  )
}
...
function renderListMeta(item) {
  var tags = item.tags || {}
  return <span>New sub-category · category #{tags.category_id || '—'}</span>
}
