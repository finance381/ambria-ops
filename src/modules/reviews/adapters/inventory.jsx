  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3 text-sm">
        ...
        <div><p className="text-[10px] font-bold text-gray-400 uppercase">Submitted</p><p className="text-gray-900">{row.created_at ? formatDate(row.created_at) : '—'}</p></div>
      </div>
      {allocs.length > 0 && (
        <div>
...
function renderListMeta(item) {
  var tags = item.tags || {}
  return <span>Qty pending · category #{tags.category_id || '—'}</span>
}
