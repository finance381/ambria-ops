...
export var PERM_GROUPS = [
  ...
  { group: 'Review', icon: '✅', scope: 'both', children: [
    ...
    { key: 'review.history', label: 'Review History', scope: 'both' },
  ]},
  ...
  { group: 'API Marketing', icon: '📣', scope: 'both', children: [
    { key: 'broadcast.templates.view', label: 'Templates', scope: 'both',
      optional: [
        { key: 'broadcast.templates.edit',   label: 'Create/edit templates' },
        { key: 'broadcast.templates.submit', label: 'Submit to Meta for approval' },
      ]},
    { key: 'broadcast.contacts.view', label: 'Contacts', scope: 'both',
      optional: [
        { key: 'broadcast.contacts.edit',    label: 'Edit contacts' },
        { key: 'broadcast.contacts.import',  label: 'Import (CSV / LMS pull)' },
        { key: 'broadcast.contacts.opt_out', label: 'Manually opt out a contact' },
      ]},
    { key: 'broadcast.campaigns.view', label: 'Campaigns', scope: 'both',
      optional: [
        { key: 'broadcast.campaigns.create', label: 'Create/edit campaigns' },
        { key: 'broadcast.campaigns.send',   label: 'Send campaigns' },
        { key: 'broadcast.campaigns.cancel', label: 'Cancel scheduled campaigns' },
      ]},
    { key: 'broadcast.inbox.view', label: 'Inbox', scope: 'both',
      note: 'Single-user inbox in v1 — this perm exists for a future multi-user rollout',
      optional: [{ key: 'broadcast.inbox.reply', label: 'Reply in inbox' }] },
    { key: 'broadcast.quicksend', label: 'Sales Quick Send', scope: 'both',
      note: 'Send only sales_approved templates from QuoteCalc/Events/Contracts — no other API Marketing access implied' },
    { key: 'broadcast.analytics.view', label: 'Analytics', scope: 'both' },
    { key: 'broadcast.settings', label: 'Settings', scope: 'desktop',
      note: 'WABA config, frequency cap, send window — admin-only in practice' },
    { key: 'broadcast.automations.view', label: 'Automations', scope: 'desktop',
      note: 'Deferred — schema scaffolded (wa_automation_rules), no consumer yet',
      optional: [{ key: 'broadcast.automations.edit', label: 'Edit automation rules' }] },
  ]},
