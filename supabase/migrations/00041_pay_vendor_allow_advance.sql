-- Allow advance payments to a vendor (paying before any bill exists, or paying
-- more than what's currently billed) — previously pay_vendor hard-blocked any
-- payment+deduction that exceeded the vendor's outstanding balance for that
-- mode, with no way to give a labour/supply vendor money up front.
--
-- Fix: only the *deduction* (a credit against an actual bill — a discount or
-- quality-issue writeoff) stays capped by what's billed and outstanding,
-- since deducting against a bill that doesn't exist makes no sense. The
-- payment itself may now exceed the outstanding balance; that's exactly what
-- an advance is, and it's expected to leave v_vendor_ledger's cash/bank
-- balance for that vendor negative (company holds credit with the vendor)
-- until future bills consume it.
--
-- Everything else in this function is unchanged from the current definition
-- (introspected directly from Supabase, since pay_vendor is created in Studio
-- and not tracked elsewhere in this repo).
...
BEGIN
  ...
  -- future bills consume it.
  IF COALESCE(p_deduction_paise, 0) > COALESCE(v_mode_balance, 0) THEN
    RAISE EXCEPTION 'pay_vendor: deduction (%) exceeds % outstanding (%)',
      COALESCE(p_deduction_paise, 0),
      p_mode,
