-- Adds an FNF (full & final) vs Advance classification to vendor payments,
-- stored in the payment ledger_entries row's own metadata (kind='payment')
-- rather than a new column — same place mode/payment_images already live.
-- Everything else in this function is unchanged from 00041_pay_vendor_allow_advance.sql.
