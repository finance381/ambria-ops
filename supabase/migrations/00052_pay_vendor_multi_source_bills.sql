-- Lets a vendor-payment deduction be attributed against multiple bills at
-- once, instead of exactly one. p_source_expense_id (bigint) is replaced
-- with p_source_expense_ids (bigint[]).
--
-- Split is two levels, same idea as the existing single-bill code just
-- applied one level higher: the deduction is first split across the
-- selected bills proportional to each bill's own amount_paise, then each
-- bill's share is split across that bill's own allocations exactly as
-- before (or credited to that bill's own expense type/sub-type if it has
-- no allocations). Remainder from rounding lands on the last bill and,
-- within a bill, the last allocation — so shares always sum exactly to
-- p_deduction_paise.
--
-- This changes an argument's TYPE (bigint -> bigint[]), which CREATE OR
-- REPLACE cannot do in place — Postgres would just add a second overload
-- and PostgREST would then fail to pick one (the exact bug 00038 fixed
-- once already for this same function). Drop the old signature first.
...
DECLARE
  ...
  v_bills_seen    integer := 0;
  v_bill_share    bigint;
  v_bill_share_sum bigint := 0;
BEGIN
  ...
  IF COALESCE(p_deduction_paise, 0) > 0 THEN
    ...
    END IF;
    IF p_source_expense_ids IS NOT NULL AND array_length(p_source_expense_ids, 1) > 0 THEN
      v_dedn_meta := v_dedn_meta || jsonb_build_object('source_expense_ids', to_jsonb(p_source_expense_ids));
    END IF;
    ...
    IF p_source_expense_ids IS NOT NULL AND array_length(p_source_expense_ids, 1) > 0 THEN
      SELECT COALESCE(SUM(e.amount_paise), 0), COUNT(*)
        INTO v_bills_total, v_bills_count
      FROM public.expenses e
      WHERE e.id = ANY(p_source_expense_ids);

      IF v_bills_count > 0 AND v_bills_total > 0 THEN
        FOR v_bill IN
          SELECT e.id, e.amount_paise, e.expense_type_id, e.expense_sub_type_id
          FROM public.expenses e
          WHERE e.id = ANY(p_source_expense_ids)
          ORDER BY e.id
        LOOP
          v_bills_seen := v_bills_seen + 1;
          IF v_bills_seen = v_bills_count THEN
            v_bill_share := p_deduction_paise - v_bill_share_sum;
          ELSE
            v_bill_share := ROUND(p_deduction_paise::numeric * v_bill.amount_paise / v_bills_total);
            v_bill_share_sum := v_bill_share_sum + v_bill_share;
          END IF;
          IF v_bill_share = 0 THEN CONTINUE; END IF;

          SELECT COALESCE(SUM(ea.amount_paise), 0), COUNT(*)
            INTO v_alloc_total, v_alloc_count
          FROM public.expense_allocations ea
          WHERE ea.expense_id = v_bill.id;
          ...
          IF v_alloc_count = 0 OR v_alloc_total <= 0 THEN
            INSERT INTO public.cost_transfers (
              ...
              v_bill_share, 'expense', v_bill.expense_type_id, v_bill.expense_sub_type_id,
              'vendor', p_vendor_id,
              'Vendor deduction credit: ' || btrim(p_deduction_reason),
              p_deduction_reason, p_entry_date, v_actor,
              jsonb_build_object('source', 'vendor_deduction', 'vendor_deduction_ledger_id', v_dedn_id, 'source_expense_id', v_bill.id)
            );
          ELSE
            v_alloc_seen := 0;
            v_share_sum := 0;
            ...
            LOOP
              ...
              IF v_alloc_seen = v_alloc_count THEN
                v_share_paise := v_bill_share - v_share_sum;
              ELSE
                v_share_paise := ROUND(v_bill_share::numeric * v_alloc.amount_paise / v_alloc_total);
